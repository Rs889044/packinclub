...
export default function ProductsContentPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/content")
      ...
      .then(data => {
        ...
        if (!data.products) data.products = { hero: { title: "", desc: "" } };
        setContent(data);
        setLoading(false);
      });
  }, []);
  ...
  const products = content.products || { hero: { title: "", desc: "" } };
    ...
    <div className="max-w-3xl">
      <div className="mb-8 p-6 bg-gradient-to-r from-brand-forest to-[#1A4533] rounded-2xl text-white shadow-lg overflow-hidden relative">
         ...
         <div className="relative z-10">
           <h1 className="font-display text-2xl md:text-3xl font-bold mb-2">Products Page Content</h1>
           <p className="text-white/80 text-sm">Edit the hero title and description shown on the public Products listing page.</p>
         </div>
      </div>

      <form onSubmit={handleSave} className="bg-white rounded-2xl border border-brand-pale p-8 space-y-6">
        <div>
          <label className="block text-sm font-semibold text-brand-charcoal mb-2">Hero Title <span className="font-normal text-brand-gray">(HTML supported — e.g. &lt;span&gt; for styling)</span></label>
          <input
            type="text"
            value={products.hero.title}
            onChange={e => setContent({...content, products: {...products, hero: {...products.hero, title: e.target.value}}})}
            placeholder="India's widest range of compostable packaging"
        ...
        <div>
          ...
          <textarea
            rows={3}
            value={products.hero.desc}
            onChange={e => setContent({...content, products: {...products, hero: {...products.hero, desc: e.target.value}}})}
            placeholder="Explore plant-based, CPCB-certified packaging products..."
